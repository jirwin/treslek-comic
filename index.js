...
Comic.prototype.comic = function(bot, to, from, msg, callback) {
  ...
  async.auto({
    img: function(callback) {
      fs.readFile(path.resolve(__dirname, './comics', comic.name + '.png'), function(err, data) {
        if (err) {
          callback(err);
          return;
        }
    ...
    upload: ['drawText', function(callback, results) {
      ...
      client = pkgcloud.storage.createClient({
        provider: 'rackspace',
        username: config.raxUser,
        apiKey: config.raxApiKey,
    ...
    bot.say(to, 'New comic uploaded at ' + config.domain + results.upload);
    callback();
  });
};
